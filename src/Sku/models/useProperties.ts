...
import { usePropertyNamesQuery, usePropertyValuesQuery } from '../../generated/graphql';
...
export const useProperties = () => {
  ...
  const [propertyNameList, setPropertyNameList] = useState<PropertyName[]>([]);
  const [propertyValueList, setPropertyValueList] = useState<PropertyValue[]>([])
  const propertyNameListRes = usePropertyNamesQuery();
  const propertyValueListRes = usePropertyValuesQuery({variables: {propertyId}})

  const fetchPropertyNameList = () => {
    const list: PropertyName[] = propertyNameListRes.data?.propertyNames as PropertyName[] || [];
    if(list !== propertyNameList){
      setPropertyNameList(list);
    }
    
  }

  useEffect(() => {
    const list: PropertyValue[] = propertyValueListRes.data?.propertyValues as PropertyValue[];
  ...
  // 更新列表
  const updatePropertiesByProperty = R.curry((property: OptionType, index: number) : any => {
    debugger;
    // 如果没有传入的规格, 则代表是删除某规格
    if(!property) {
      return deleteProperty(index);
    } else {
      console.log('updatePropertiesByProperty', property)
